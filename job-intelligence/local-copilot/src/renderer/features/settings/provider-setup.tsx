...
export function ProviderSetup({ state, controller }: { readonly state: CopilotUiState; readonly controller: CopilotController }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    void controller.saveProviderSecret(providerId, secret).finally(() => setSecret(''));
  };
  ...
  return <section className="panel provider-setup" aria-labelledby="provider-heading">
    ...
    <ul className="disclosures" aria-label="Provider destinations">{state.providers.map((provider) => <li key={provider.id}><span>{provider.name}</span><small>{provider.destination}</small><span className="status">{provider.configured ? 'Key saved' : 'Not configured'}</span></li>)}</ul>
    <form onSubmit={submit} className="secret-form">
      ...
      <div className="button-row"><button className="primary" type="submit">Save key</button></div>
