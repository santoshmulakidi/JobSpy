...
export function AnswerPanel({ state, controller }: { readonly state: CopilotUiState; readonly controller: CopilotController }) {
  return <section className="panel answer-panel" aria-labelledby="answer-heading">
    ...
    <div className="button-row"><button type="button" disabled={!state.answerPending} onClick={() => void controller.cancelAnswer()}>Cancel</button><button type="button" disabled={!state.answerConnected || state.answerPending || !state.transcriptDraft.trim()} onClick={() => void controller.retryAnswer()}>Retry</button></div>
  </section>;
}
