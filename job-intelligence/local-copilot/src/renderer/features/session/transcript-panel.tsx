...
export function TranscriptPanel({ state, controller }: { readonly state: CopilotUiState; readonly controller: CopilotController }) {
  return <section className="panel" aria-labelledby="transcript-heading">
    ...
    <textarea id="transcript" aria-live="polite" rows={3} value={state.transcriptDraft} placeholder="Captured speech appears here…" onChange={(event) => controller.editTranscript(event.currentTarget.value)} />
    <button className="primary" type="button" disabled={!state.answerConnected || state.answerPending || !state.transcriptDraft.trim()} onClick={() => void controller.sendQuestion()}>Send question</button>
